...
/* Read a temp's value into a data register; return that register's name.
 * If the temp is spilled, loads from the spill slot into dregs[scratch]. */
static const char *
...
/* Pick the register that will hold a temp's defined value. */
static const char *
...
/* Emit the spill-store for a defined temp, if necessary. */
static void
...
/* Emits:  move.l A, dst ; <mnem> B, dst  (with scratch juggling so B
 * is preserved if it aliases dst). */
static void
         ...
         const char *scc)
{
    ...
    sb = rs(out, fn, i->b, 1);
    sd = rd(fn, i->dst, 0);
    /* cmp.l src, dst computes dst - src; result reflects A op B when
     * we write cmp.l <B>, <A>. */
    /* Clear dst BEFORE cmp — moveq clobbers condition codes, so doing
     * it after the compare would destroy the flags we need for Scc.
     * Scc writes only the low byte (0 or 0xFF); negating the byte turns
     * that into 0 or 1 while leaving the zeroed upper bits intact. */
...
static int narg;
static int label_prefix;        /* unique per-function to keep .L labels distinct */
...
static void
emit_store(FILE *out, struct ir_func *fn, struct ir_insn *i, const char *sz)
{
    const char *sa, *sb;

    sa = rs(out, fn, i->a, 0);  /* address */
    sb = rs(out, fn, i->b, 1);  /* value */
    fprintf(out, "\tmovea.l %s, %%a0\n", sa);
...
emit_prologue(FILE *out, struct ir_func *fn)
{
    ...
    /* link.w displacement is 16-bit signed; negative to allocate. */
...
static void
emit_call_flush(FILE *out, struct ir_func *fn, struct ir_insn *i, int indirect)
{
    int k;
...
emit_insn(FILE *out, struct ir_func *fn, struct ir_insn *i)
{
    ...
    case IR_MODS:
    case IR_MODU: {
        /* q = a / b ; r = a - q*b.  Use d0 and d1 as scratch. */
        const char *sa = rs(out, fn, i->a, 0);
        const char *sb = rs(out, fn, i->b, 1);
        const char *sd = rd(fn, i->dst, 0);
        const char *divop = (i->op == IR_MODS) ? "divs.l" : "divu.l";
        /* load sb into d1 if it would be clobbered (it's read twice). */
        if (strcmp(sb, "%d1") != 0) {
            fprintf(out, "\tmove.l %s, %%d1\n", sb);
            sb = "%d1";
        }
    ...
    case IR_FUNC:
    case IR_ENDF:
        /* Function framing is handled in emit_function. */
        break;
...
emit_function(FILE *out, struct ir_func *fn)
{
    ...
    /* Safety net: if the body somehow fell through without a RET, emit one. */
...
emit_globals(FILE *out, struct ir_program *prog)
{
    ...
    for (g = prog->globals; g; g = g->next) {
        int is_char_arr = (g->base_type == T_CHAR && g->arr_size > 0);

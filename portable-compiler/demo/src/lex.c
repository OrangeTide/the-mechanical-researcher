 ...
 *
 * Numbers: decimal or hex (0x...). Char literals and strings support
 * \n \t \r \0 \\ \' \" and \xNN. Line comments start with //; block
 * comments are the usual C form (no nesting).
 */
...
lex_one(void)
{
    ...
    if (c == '\'') {
        ...
        if (*p != '\'')
            die("%s:%d: unterminated char literal", src_file, line);
        p++;
        t = make(T_CHARLIT);
        t.nval = n;
        return t;

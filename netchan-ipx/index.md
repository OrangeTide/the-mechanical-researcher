...
Movement is tile-stepped at the tick rate, which is authentic to the Kroz/Caves-of-Thor text-tile lineage and is the simplest thing that looks right. On DOS the screen is written straight to the `0xB800` text buffer; input comes from a custom `INT 09h` keyboard handler that tracks key state, so holding an arrow gives smooth eight-direction movement that the BIOS keystroke queue cannot. The same game compiles on the host with an ANSI/`termios` backend, which is how the screenshot above was produced.
